...
test(@(x) extended_rosenbrock(x), @(x) extended_rosenbrock_gradf(x),...
    ...
    @extended_rosenbrock_gradf_fd, @extended_rosenbrock_Hessf_fd, codiags,...
...
test(@(x) generalized_broyden(x), @(x) generalized_broyden_gradf(x),...
    ...
    @generalized_broyden_gradf_fd, @generalized_broyden_Hessf_fd, codiags,...
...
test(@(x) banded_trigonometric(x), @(x) banded_trigonometric_gradf(x),...
    ...
    @banded_trigonometric_gradf_fd, @banded_trigonometric_Hessf_fd, codiags,...
    kmax, 1e-6, 1e-2, 0.5, 50, 100, 1e-3, fterm, 100, '../results/banded_trigonometric/', true, 2);